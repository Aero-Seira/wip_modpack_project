...
ItemEvents.entityInteracted("rimcraft:blood_collection_device", e => {
    if (e.player.cooldowns.isOnCooldown("rimcraft:blood_collection_device"))
        return
    let entity = e.target
    if (!entity || !entity.living)
        return
    //限制主手使用
    if (!(e.player.mainHandItem == "rimcraft:blood_collection_device" && e.player.offHandItem == "minecraft:glass_bottle"))
        return
