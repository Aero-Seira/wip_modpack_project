ServerEvents.recipes(e => {
    e.forEachRecipe({ type: "vinery:wine_fermentation" }, r => {
        ...
        let resFluid = Fluid.of(`rimcraft:${res.id.toString().split(":")[1]}`, 1000)
        let resFluid_250 = Fluid.of(`rimcraft:${res.id.toString().split(":")[1]}`, 250)
        e.recipes.create.filling(res, ["vinery:wine_bottle", resFluid_250])
            .id(`rimcraft:filling/${res.id.toString().split(":")[1]}`)
        e.recipes.create.emptying(["vinery:wine_bottle", resFluid_250], res)
            .id(`rimcraft:emptying/${res.id.toString().split(":")[1]}`)
        pouring(e, res, resFluid.id, "vinery:wine_bottle")
        let fluid = recipe.getJuiceType().split("_")
        let fluidId = ""
        let prefix = fluid[0]
        if (fluid[1] == "general") {
            prefix = "rimcraftcore:" + fluid[0]
            fluidId = prefix + "_grapejuice"
        }
        else if (fluid[1] == "warped" || fluid[1] == "crimson") {
            prefix = "rimcraftcore:" + fluid[1]
            fluidId = prefix + "_grapejuice"
        }
        else if (fluid[0] == "apple") {
            fluidId = "rimcraft:apple_juice"
        }
        else if (fluid[1]) {
            prefix = `${fluid[1]}_${prefix}`
            prefix = "rimcraftcore:" + prefix
            fluidId = prefix + "_grapejuice"
        }
        ...
        recipe.ingredients.forEach(i => {
            i.stacks.forEach(ingr => {
                ...
                else if (!Fluid.of(`rimcraft:${ingr.id.split(":")[1]}`).empty)
                    ingrs.push(Fluid.of(`rimcraft:${ingr.id.split(":")[1]}`, 250))
                else
    ...
    e.recipes.create.compacting(Fluid.of("rimcraft:apple_juice", 250), "vinery:apple_mash")
        .id("rimcraft:compacting/apple_juice")
    e.recipes.create.filling("vinery:apple_juice", ["vinery:wine_bottle", Fluid.of("rimcraft:apple_juice", 250)])
        .id("vinery:filling/apple_juice")
    e.recipes.create.emptying(["vinery:wine_bottle", Fluid.of("rimcraft:apple_juice", 250)], "vinery:apple_juice")
        .id("vinery:emptying/apple_juice")
    pouring(e, 'vinery:apple_juice', "rimcraft:apple_juice", "vinery:wine_bottle", 250)
})