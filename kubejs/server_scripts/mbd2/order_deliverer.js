...
MBDMachineEvents.onTick("rimcraft:order_deliverer", e => {
    let event = e.event
    const { machine } = event
        ...
        if (level.dayTime() % 24000 == 1000 + 5 * 20) {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                for (let index = 1; index <= count; index++) {
                    ...
                    let find = item.find(item => item.is("rimcraft:order"))

                    if (find != null) {
                        if (order != null) {
                            let o = find.nbt.rimcraftOrderInfo
                            ...
                            if (reward == null)
                                reward = [`rimcraft:orders/${o.type}`, 1] //如果没有写reward那么则为以类型为名的战利品表
                ...
                if (order != null) {
                    let o = order.nbt.rimcraftOrderInfo
                    let nums = Order.checkAllPackages([o], packages)
                    let customer = Order.customerProperties[o.type]
                    let reward = customer.reward
                    if (reward == null)
                        reward = [`rimcraft:orders/${o.type}`, 1] //如果没有写reward那么以类型为名的战利品表
                    let list = Utils.newList()
            ...
            //     if (item.is("rimcraft:order"))
            //         orders.push(item.nbt.rimcraftOrderInfo)
            //     else
            //         orders.push(null)
            // }
...
BlockEvents.rightClicked("create:white_table_cloth", e => {
    ...
    if (player.mainHandItem.is("rimcraft:order_deliverer_item")) {
        block.set("rimcraft:order_deliverer")
...
LootJS.modifiers(e => {
    e.addBlockLootModifier("rimcraft:order_deliverer")
        .addLoot("create:white_table_cloth")
        .addLoot("rimcraft:order_deliverer_item")
})