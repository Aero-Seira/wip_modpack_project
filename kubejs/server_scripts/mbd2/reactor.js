...
//初始化CustomData
MBDMachineEvents.onStructureFormed("rimcraft:fission_reactor", e => {
    let customData = e.event.machine.customData
        ...
        multiblock.parts.forEach(part => {
            ...
            if (machinePart.definition.id() == "rimcraft:fission_fuel_assembly")
                assembly_count++
        })
...
//升温逻辑及代码判断
MBDMachineEvents.onTick("rimcraft:fission_reactor", e => {
...
//温度对于产电的调节
MBDMachineEvents.onBeforeRecipeModify("rimcraft:fission_reactor", e => {
...
MBDMachineEvents.onAfterRecipeModify("rimcraft:fission_reactor", e => {
    const { machine, recipe } = e.event
    let multiplier = machine.customData.getDouble("multiplier") 
...
MBDMachineEvents.onAfterRecipeWorking("rimcraft:fission_reactor", e => {
    const { machine, recipe } = e.event
    
    if (recipe.id && recipe.id == "rimcraft:fission_react/empty") {
        ...
        // machine.level.recipeManager.getByType("rimcraft:fission_react").forEach((id, r) => {
...
//实现移除时核反应堆爆炸
MBDMachineEvents.onRemoved("rimcraft:fission_reactor", e => {
...
MBDMachineEvents.onStateChanged("rimcraft:fission_reactor", e => {
...
MBDMachineEvents.onStructureInvalid("rimcraft:fission_reactor", e => {
    // console.log(`machineStateName: ${e.event.machine.machineStateName}`)
    if (e.event.machine.machineStateName == "working") {
        ReactorUtil.Bombbbb(e.event.machine)
    }
...
//实现机器紧急启停效果
    //UI实现
MBDMachineEvents.onUI("rimcraft:fission_reactor", e => {
    ...
    damage.setTextProvider(() => 
        Component.translate("message.rimcraft.fission_reactor.degree_of_damage", Math.round(customData.getDouble("degree_of_damage") * 100) / 100)
    )

    temperature.setTextProvider(() => 
        Component.translate("message.rimcraft.fission_reactor.temperature", Math.round(customData.getDouble("temperature") * 100) / 100)
    )

    reactor_switch.setTextProvider(() => 
        Component.translate("message.rimcraft.fission_reactor.switch").append(
            customData.getDouble("temperature") >= 1500 
                ? Component.translate(
                    customData.getDouble("temperature") >= 2000 
                        ? "message.rimcraft.fission_reactor.meltdown_temp" 
                        : "message.rimcraft.fission_reactor.critical_temp"
                )
                : Component.translate(
                    customData.getDouble("burning_rate") != 0
                        ? "message.rimcraft.fission_reactor.switch_on" 
                        : "message.rimcraft.fission_reactor.switch_off"
                )
        )
    )
    ...
    heatProduced.setTextProvider(() =>
        Component.translate("message.rimcraft.fission_reactor.heat_produced", Math.round(ReactorUtil.heatProduced(machine) * 100) / 100)
    )

    cooling.setTextProvider(() =>
        Component.translate("message.rimcraft.fission_reactor.fluid_cooling", Math.round(ReactorUtil.cooling(machine)[0] * 100) / 100)
    )

    ambientCooling.setTextProvider(() =>
        Component.translate("message.rimcraft.fission_reactor.ambient_cooling",
            Math.round((0.05 + (customData.getDouble("temperature") - 298.15) * 0.0001) * 100) / 100
        )
    )

    inputFluid.setTextProvider(() =>
        Component.translate("message.rimcraft.fission_reactor.input_fluid", Math.round(ReactorUtil.inputFluid(machine,) * 100) / 100)
    )

    outputFluid.setTextProvider(() =>
        Component.translate("message.rimcraft.fission_reactor.output_fluid", Math.round(ReactorUtil.outputFluid(machine) * 100) / 100)
    )

    outputEnergy.setTextProvider(() =>
        Component.translate("message.rimcraft.fission_reactor.output_energy", Math.round(ReactorUtil.outputEnergy(machine)) / 1000)
    )

    burningRate.setTextProvider(() =>
        Component.translate("message.rimcraft.fission_reactor.burning_rate", Math.round(customData.getDouble("burning_rate") * 100))
    )
})
    //配方逻辑
MBDMachineEvents.onBeforeRecipeWorking("rimcraft:fission_reactor", e => {
    const { machine } = e.event
    if (machine.customData.getDouble("burning_rate") == 0) {
        e.event.setCanceled(true)
    }
})
MBDMachineEvents.onFuelRecipeModify("rimcraft:fission_reactor", e => {
    const { machine } = e.event
    if (machine.customData.getDouble("burning_rate") == 0) {
        e.event.setCanceled(true)
    }
...
// MBDMachineEvents.onTick("rimcraft:fission_fuel_assembly", e => {
