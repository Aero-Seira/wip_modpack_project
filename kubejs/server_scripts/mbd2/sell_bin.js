...
MBDMachineEvents.onTick("rimcraft:sell_bin", e => {
    ...
    if (!coinValue.empty) {
        if (player != null) {
            if (player instanceof $ServerPlayer) {
                ...
                severPlayer.connection.send(new $ClientboundSetTitleTextPacket(Component.translate("message.rimcraft.sell_bin_hint").color(Color.GOLD)))
            ...
            let total = Component.translate("message.rimcraft.sell_bin_total", MoneyUtil.convertBaseValueToString(values))
            player.tell(total)
            player.tell(Component.of("§e-----------------------"))
...
//初始化存储玩家数据
MBDMachineEvents.onPlaced("rimcraft:sell_bin", e => {
    const {machine} = e.event
...
//     let coinValue = $CoinValue["fromNumber(java.lang.String,long)"](COIN_CHAIN_MAIN_VALUE, values)
//     if (!coinValue.empty)
//         player.tell(Component.translate("message.rimcraft.sell_bin_hint", machine.pos.toShortString(), coinValue.getText().getString()))
//     $MoneyAPI.API.GetPlayersMoneyHandler(player).insertMoney(coinValue, false)
