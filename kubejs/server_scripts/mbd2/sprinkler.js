MBDMachineEvents.onPlaced("rimcraft:sprinkler", e => {
...
MBDMachineEvents.onTick("rimcraft:sprinkler", e => {
    const { machine } = e.event
    const {customData, level} = machine
...
MBDMachineEvents.onRemoved("rimcraft:sprinkler", e => {
    const { machine } = e.event
    const {customData, level} = machine
    let manager = SeasonUtil.getSaveData(level)
    if (manager != null) {
...
MBDMachineEvents.onRecipeWorking("rimcraft:sprinkler", e => {
    const { machine } = e.event
    const {level} = machine
