MBDMachineEvents.onAfterRecipeModify("rimcraft:contract_executor", e => {
    let event = e.event
...
MBDMachineEvents.onUI("rimcraft:contract_executor", e => {
    let event = e.event
    const {machine, root} = event
    let text = root.getFirstWidgetById("blaze_burner_count")
    
    text.setTextProvider(() => Component.translate("message.rimcraft.blaze_burner_count",
