...
ItemEvents.rightClicked((event) => {
  ...
  if (player.crouching && player.mainHandItem !== "minecraft:air" && isDeveloper(player.username) && player.mainHandItem !== "rimcraft:debug_reload_tool" && player.mainHandItem!== "rimcraft:debug_info_tool") {
...
PlayerEvents.loggedIn((event) => {
  const { player } = event;
  if (isDeveloper(player.username)) {
    player.tell(Text.translate("message.rimcraft.debug", [player.username]));
    player.tell(Text.translate("message.rimcraft.conventcommand", [player.username]));
  }
});
