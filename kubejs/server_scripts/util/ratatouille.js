...
function freezing(event, ingredients, results, time) {
  time = time || 200
  event.recipes.ratatouille.freezing(results, ingredients).id(`rimcraftcore:freezing/${results.split(":")[1]}`)
  event.recipes.rimcraftcore.fan_freezing(results, ingredients).id(`rimcraftcore:fan_freezing/${ingredients.split(":")[1]}`)
