...
Ponder.registry(event => {
    ...
    event.create([
        ...
        .scene(
            "rimcraft:fission_reactor",
            "裂变反应堆的搭建",
            "rimcraft:ponder_fission_reactor",
            ...
            (builder, util) => {
                ...
                scene.world.replaceBlocks([2, 1, 6], Block.id("rimcraft:forged_steel_export_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.WEST), true)
                scene.world.replaceBlocks([2, 2, 6], Block.id("rimcraft:forged_steel_export_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.WEST), true)
                scene.idle(40)
                scene.rotateCameraY(90)
                scene.text(40, "在这个多方块结构中, 所有的锻造钢机壳都可以被锻造钢输入/输出总线代替, 也可以替换成逻辑端口来进行控制", [11, 2.5, 6.5])
                scene.world.replaceBlocks([10, 1, 6], Block.id("rimcraft:forged_steel_import_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.EAST), true)
                scene.world.replaceBlocks([10, 2, 6], Block.id("rimcraft:forged_steel_import_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.EAST), true)
                scene.idle(40)
                scene.rotateCameraY(-180)
                scene.text(40, "在这个多方块结构中, 所有的锻造钢机壳都可以被锻造钢输入/输出总线代替, 也可以替换成逻辑端口来进行控制", [6.5, 2.5, 11])
                scene.world.replaceBlocks([6, 1, 10], Block.id("rimcraft:forged_steel_import_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.SOUTH), true)
                scene.world.replaceBlocks([6, 2, 10], Block.id("rimcraft:forged_steel_import_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.SOUTH), true)
                scene.idle(40)
                scene.rotateCameraY(90)
                    ...
                    for (let x = 3; x < 10; x++) {
                        for (let y = 3; y < 10; y++) {
                            ...
                            if (!excludedPos.some(pos => pos[0] == x && pos[1] == y)) {
                                scene.world.setBlocks([x, z, y], Block.id("rimcraft:fission_fuel_assembly").blockState, true)
                                scene.idle(1)
                            }
                        }
                    }
