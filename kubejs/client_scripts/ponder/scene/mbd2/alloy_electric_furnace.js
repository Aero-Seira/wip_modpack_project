...
Ponder.registry(e => {
    e.create([
        ...
        .scene(
            "rimcraft:alloy_electric_furnace",
            "合金电炉的搭建",
            "rimcraft:ponder_alloy_electric_furnace",
            ...
            (builder, util) => {
                ...
                scene.world.replaceBlocks([2, 2, 3], Block.id("rimcraft:forged_steel_export_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.WEST), true)
                scene.world.replaceBlocks([2, 2, 4], Block.id("rimcraft:forged_steel_export_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.WEST), true)
                scene.world.replaceBlocks([2, 2, 5], Block.id("rimcraft:forged_steel_export_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.WEST), true)
                scene.idle(40)
                scene.rotateCameraY(90)
                scene.text(40, "在这个多方块结构中, 任意的锻造钢机壳都可以被锻造钢输入/输出总线代替", [7, 2.5, 4.5])
                scene.world.replaceBlocks([6, 2, 3], Block.id("rimcraft:forged_steel_import_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.EAST), true)
                scene.world.replaceBlocks([6, 2, 4], Block.id("rimcraft:forged_steel_import_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.EAST), true)
                scene.world.replaceBlocks([6, 2, 5], Block.id("rimcraft:forged_steel_import_bus").blockState.trySetValue($BlockStateProperties.FACING, Direction.EAST), true)
                scene.idle(40)
        ...
        .scene(
            "rimcraft:using_alloy_electric_furnace",
            "合金电炉的使用",
            "rimcraft:ponder_alloy_electric_furnace_use",
            (builder, util) => {
                let scene = new $CreateSceneBuilder(builder)
                scene.showBasePlate()
                scene.idle(20)
