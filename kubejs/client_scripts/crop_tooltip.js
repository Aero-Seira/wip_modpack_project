    ...
    e.addAdvancedToAll((item, advanced, text) => {
        ...
        if (item.hasNBT() && item.nbt.contains("quality_food")) {
            if (tags.anyMatch(tag => tag.location().toString().startsWith("eclipticseasons")))
                text.add(Text.translate("tooltip.rimcraft.quality_crop_grow"))
            if (item.edible)
                text.add(Text.translate("tooltip.rimcraft.quality_food_eaten"))
            text.add(Text.translate("tooltip.rimcraft.quality_sell"))
        }
    })
})