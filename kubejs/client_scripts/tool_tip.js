...
ItemEvents.tooltip(e => {
    ...
    addShiftTooltip(e, [
        ...
        'bakeries:tiramisu',
        'bakeries:cake_roll',
        'rimcraft:unactivated_crystalline_flower',
...
let tooltips = [
    ...
    ['rimcraft:debug_reload_tool', "蹲下右键重载server脚本， 站立右键重载client脚本", "EMM"],
    ['rimcraft:debug_info_tool', "蹲下右键获取当前结构名称， 站立右键获取指向方块实体信息（未完成）", "EMM"],
...
ItemEvents.tooltip(e => {
    // 磁铁
    e.add("alexscaves:block_of_azure_neodymium", Text.translate("tooltip.rimcraft.magnetic_flux_density"))
    e.add("alexscaves:block_of_scarlet_neodymium", Text.translate("tooltip.rimcraft.magnetic_flux_density"))
...
//             if (!e.shift) {
//                 text.add(Component.translate("tooltip.rimcraft.single_price", trade[1].toString()))
//             } else {
//                 text.add(Component.translate("tooltip.rimcraft.total_price", MoneyUtil.convertBaseValueToString(trade[1] * item.count)))
//             }
//         })
//     })
// })
...
ItemEvents.tooltip(e => {
    e.addAdvancedToAll((item, advanced, text) => {
        ...
        if (value > 0) {
            if (!e.shift) {
                if (value < 1) {
                    text.add(Component.translate("tooltip.rimcraft.single_price", (Math.round(value * 10) / 10).toString()).append(MoneyUtil.convertBaseValueToString(-1)))
                } else {
                    text.add(Component.translate("tooltip.rimcraft.single_price", MoneyUtil.convertBaseValueToString(value)))
                }
            } else {
                if (value * item.count < 1) {
                    text.add(Component.translate("tooltip.rimcraft.total_price", (Math.round(value * item.count * 10) / 10).toString()).append(MoneyUtil.convertBaseValueToString(-1)))
                } else {
                    text.add(Component.translate("tooltip.rimcraft.total_price", MoneyUtil.convertBaseValueToString(value * item.count)))
                }
            }
        }
    })
    e.addAdvanced("rimcraft:sell_bin", (item, advanced, text) => {
            if (!e.ctrl) {
                text.add(1, Text.translatable("tooltip.rimcraft.hold_ctrl_to_see_more_info"))
            } else {
                text.add(1, Text.translatable("tooltip.rimcraft.hold_ctrl"))
                text.add(2, Text.translatable(`tooltip.rimcraft.ctrl_${item.getId().split(":")[1]}`, MoneyUtil.convertBaseValueToString(1)))
            }
    })
    e.addAdvancedToAll((item, advanced, text) => {
        if (item.hasNBT() && item.nbt.contains("SequencedAssembly") && !(item.item instanceof $SequencedAssemblyItem)) {
            text.add(Text.translatable("tooltip.rimcraft.sequenced_assembly_explanation"))
        }
    })
        ...
        element.forEach(val => {
            ...
            e.addAdvanced(key, (item, advanced, text) => {
                text.add(Text.translatable("tooltip.rimcraft.difficulty_loot", Text.translatable(`entity.${entitys[0]}.${entitys[1]}`), val.difficulty.toFixed()))
            })
        })
    }
})