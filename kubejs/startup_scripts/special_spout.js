...
function getFuelMultipler(fluid) {
    ...
    if (fluid.id == "minecraft:lava" || fluid.id == "minecraft:water")
        ...
        fluidMultipler = 0.4
    else if (fluid.id == "rimcraft:fuel_mixtures")
        fluidMultipler = 0.25
    else
        fluidMultipler = 0
...
CreateEvents.spoutHandler(e => {
    ...
    // e.add(
    //     "rimcraft:depot_filling_tank_spout_interaction",
    ...
    // e.add("rimcraft:budding_certus_fix_spout_interaction",
    //     ["ae2:chipped_budding_quartz", "ae2:damaged_budding_quartz", "ae2:quartz_block"],
    //     (block, fluid, simulate) => {
    //         if (fluid.id == "rimcraft:spent_liquor" && fluid.amount >= 50 && block.level.random.nextInt(4) == 0) {
    //             if (!simulate) {
    ...
    // )
    e.add(
        "rimcraft:sulfur_spout_interaction", // ID
