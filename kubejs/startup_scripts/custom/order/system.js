    ...
    orders.forEach(order => {
        ...
        needed.forEach(req => {
            for (let i = 0; i < transfer.getSlots(); i++) {
                ...
                if (!stack.isEmpty() && stack.hasTag("rimcraft:order/" + req.id)) {
...
Order.getGoodsOrderProperty = function (item, type) {
    let goodsMap = RimCraft.goodsMap.get(type)
    if (goodsMap != null)
    ...
    if (!item.hasTag("rimcraft:order/" + type))
        return
...
Order.getRewardContract = function (type, count) {
    let reward = Item.of('lightmanscurrency:ticket', count, `{ 
        ...
        TicketID: -10 }`)
    reward.setHoverName(Component.translate("item.rimcraft.name." + type).italic(false))
    return reward
}

Order.addOrderToAuction = function() {
    let data = new $AuctionTradeData({})
    data.auctionItems.add(Item.of("rimcraft:unopened_order"))
    data.setMinBidDifferent($CoinValue.fromItemOrValue("createdeco:copper_coin", 1))
    data.setStartingBid($CoinValue.fromItemOrValue("rimcraftcore:gold_coin", 1).multiplyValue(Utils.random.nextFloat(0.5, 2)))
...
//     Utils.server.lootData.getLootTable("rimcraft:orders/random_hatbag").getRandomItems(param).forEach(item => player.give(item))
//     player.tell(123)
// })